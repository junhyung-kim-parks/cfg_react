...
export function RouteSidebar() {
  ...
  useEffect(() => {
    ...
    if (isFormGeneratorRoute && !expandedItems.includes('form-generator')) {
      setExpandedItems(prev => [...prev, 'form-generator']);
    }
  ...
  const requiresProjectSelection = (href: string) => {
    ...
    return projectRequiredRoutes.some(route => href.startsWith(route));
  };
  ...
  const handleNavigation = (href: string) => {
    ...
    if (isNavigatingAwayFromFlow && hasUnsavedData()) {
      console.log('🛡️ RouteSidebar: Navigation guard triggered', {
        currentPath,
        targetHref: href,
        isFormGeneratorFlow,
        ...
        {item.href ? (
          ...
          >
            <item.icon className={cn(
              ...
              requiresAuthentication(item.href!) && !isLoggedIn && "text-muted-foreground"
            )} />
